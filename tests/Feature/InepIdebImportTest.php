...
test('official municipal IDEB workbooks preserve missing and not applicable results', function () {
    ...
    expect($observations)->toHaveCount(8)
    ...
    expect(SourceRelease::query()->count())->toBe(2);
    ...
    expect(IndicatorObservation::query()->count())->toBe(8)
        ->and(SourceRelease::query()->count())->toBe(2);
    Http::assertSentCount(4);
});

/**
 * @param  list<array{code: string, 2021: string, 2023: string}>  $rows
 */
...
function idebWorksheet(array $rows): string
{
    $xml = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
        ...
        .'<c r="E10" t="s"><v>2</v></c><c r="F10" t="s"><v>3</v></c></row>';

    foreach ($rows as $index => $row) {
        $number = $index + 11;
        $xml .= '<row r="'.$number.'"><c r="A'.$number.'"><v>'.$row['code'].'</v></c>'
            .'<c r="D'.$number.'" t="s"><v>4</v></c>'
            .idebResultCell('E', $number, $row[2021])
            .idebResultCell('F', $number, $row[2023]).'</row>';
    }
...
{
    if ($value === '-') {
        return '<c r="'.$column.$row.'" t="s"><v>5</v></c>';
    }
